...
matcher.enablePlots(false);
        ...
        matcher.setEcalChannels(this.getConditionsManager().getCachedConditions(EcalChannelCollection.class, "ecal_channels").getCachedData());
...
//        tupleWriter.println();
    ...
    protected void addParticleVariables(String prefix) {
        String[] newVars = new String[]{"PX/D", "PY/D", "PZ/D", "P/D",
            ...
            "MatchChisq/D", "MatchChisqCalc/D", "ClT/D", "ClE/D", "ClSeedE/D", "ClX/D", "ClY/D", "ClZ/D", "ClHits/I", "Clix/I","Cliy/I"};
...
//    }
    ...
    protected TrackState fillParticleVariables(EventHeader event, ReconstructedParticle particle, String prefix) {
        ...
        if (!particle.getTracks().isEmpty()) {
            ...
            Hep3Vector atEcal = TrackUtils.getTrackPositionAtEcal(tweakedTrackState);
            //Hep3Vector atEcal = CoordinateTransformations.transformVectorToDetector(new BasicHep3Vector(TrackUtils.getTrackStateAtECal(track).getReferencePoint()));
            
            
          //  System.out.println(atEcal + " " + TrackUtils.getTrackPositionAtEcal(tweakedTrackState));
            ...
            double goodnessPID = 999999;
            if(particle.getClusters().size() != 0)
                goodnessPID = matcher.getNSigmaPosition(particle.getClusters().get(0), particle);
            ...
            tupleMap.put(prefix + "MatchChisq/D", goodnessPID);
            double goodnessPID_calc = 9999999;
            if(particle.getClusters().size() != 0){
                TrackClusterMatch_calc calc = new TrackClusterMatch_calc();
                double p = pRot.magnitude();
                
                Cluster cluster = particle.getClusters().get(0);
                
                //find the uncorrected version of this cluster
                for(Cluster c : event.get(Cluster.class, "EcalClusters")){
                    if(c.getCalorimeterHits().get(0).getCellID() == cluster.getCalorimeterHits().get(0).getCellID())
                    {
                        cluster = c;
                        break;
                    }
                ...
                double nsx = calc.n_sigma_x(p, cluster.getPosition()[0]
                    ,atEcal.x(), iso[10] != null, (int)particle.getCharge()
                    , particle.getTracks().get(0).getTrackStates().get(0).getTanLambda() >0);
                double nsy = calc.n_sigma_y(p, cluster.getPosition()[1]
                        ,atEcal.y(),atEcal.x(), iso[10] != null, (int)particle.getCharge()
                        , particle.getTracks().get(0).getTrackStates().get(0).getTanLambda() >0);
                goodnessPID_calc = Math.hypot(nsx, nsy);
            }
            tupleMap.put(prefix + "MatchChisqCalc/D", goodnessPID_calc);
